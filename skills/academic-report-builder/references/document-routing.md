...
- Choosing `template: unl` on a non-academic route still renders the institutional shell (logo, title, `AUTOR:`, place and date), but omits the framed subject/activity/parallel box and the `DOCENTE:` block entirely, since those fields are academic-only furniture the route forbids.
